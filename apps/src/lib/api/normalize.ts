...
export function normalizeAccount(item: unknown, usage?: AccountUsage | null): Account | null {
  ...
  return {
    ...
    usagePrimaryWindowRequestCount: Math.max(0, asInteger(source.usagePrimaryWindowRequestCount ?? source.usage_primary_window_request_count, 0, 0)),
    usagePrimaryWindowTotalTokens: Math.max(0, asInteger(source.usagePrimaryWindowTotalTokens ?? source.usage_primary_window_total_tokens, 0, 0)),
    usagePrimaryWindowEstimatedCostUsd: Math.max(0, toNullableNumber(source.usagePrimaryWindowEstimatedCostUsd ?? source.usage_primary_window_estimated_cost_usd) ?? 0),
    usageSecondaryWindowRequestCount: Math.max(0, asInteger(source.usageSecondaryWindowRequestCount ?? source.usage_secondary_window_request_count, 0, 0)),
    usageSecondaryWindowTotalTokens: Math.max(0, asInteger(source.usageSecondaryWindowTotalTokens ?? source.usage_secondary_window_total_tokens, 0, 0)),
    usageSecondaryWindowEstimatedCostUsd: Math.max(0, toNullableNumber(source.usageSecondaryWindowEstimatedCostUsd ?? source.usage_secondary_window_estimated_cost_usd) ?? 0),
