...
DROP TABLE IF EXISTS temp.cm_gpt6_sol_luna_request_repriced;
DROP TABLE IF EXISTS temp.cm_gpt6_sol_luna_rollup_repriced;
DROP TABLE IF EXISTS temp.cm_gpt6_sol_luna_account_cost_deltas;

CREATE TEMP TABLE cm_gpt6_sol_luna_request_repriced AS
WITH source AS (
    SELECT
        t.id,
    ...
    SELECT *
    FROM source
    WHERE normalized_model IN ('gpt-6-sol', 'gpt-6-luna')
       OR normalized_model LIKE 'gpt-6-sol-%'
       OR normalized_model LIKE 'gpt-6-luna-%'
       OR normalized_model LIKE '%/gpt-6-sol'
       OR normalized_model LIKE '%/gpt-6-sol-%'
       OR normalized_model LIKE '%/gpt-6-luna'
       OR normalized_model LIKE '%/gpt-6-luna-%'
...
SELECT
    *,
    CASE
        WHEN normalized_model LIKE '%gpt-6-sol%' THEN
            (((input_tokens - cached_input_tokens)
                * CASE WHEN input_tokens > 272000 THEN 4.0 ELSE 2.0 END)
              + (cached_input_tokens
                * CASE WHEN input_tokens > 272000 THEN 0.4 ELSE 0.2 END)
              + (output_tokens
                * CASE WHEN input_tokens > 272000 THEN 15.0 ELSE 10.0 END))
              * tier_multiplier / 1000000.0
        ELSE
            (((input_tokens - cached_input_tokens)
                * CASE WHEN input_tokens > 272000 THEN 0.2 ELSE 0.1 END)
              + (cached_input_tokens
                * CASE WHEN input_tokens > 272000 THEN 0.02 ELSE 0.01 END)
              + (output_tokens
                * CASE WHEN input_tokens > 272000 THEN 0.75 ELSE 0.5 END))
              * tier_multiplier / 1000000.0
    END AS new_cost_usd
FROM matched;

CREATE TEMP TABLE cm_gpt6_sol_luna_rollup_repriced AS
WITH source AS (
    SELECT
        key_id,
    ...
    CASE
        WHEN normalized_model LIKE '%gpt-6-sol%' THEN
            ((MAX(input_tokens - cached_input_tokens, 0)
                * CASE WHEN input_tokens > 272000 THEN 4.0 ELSE 2.0 END)
              + (MIN(MAX(cached_input_tokens, 0), MAX(input_tokens, 0))
                * CASE WHEN input_tokens > 272000 THEN 0.4 ELSE 0.2 END)
              + (MAX(output_tokens, 0)
                * CASE WHEN input_tokens > 272000 THEN 15.0 ELSE 10.0 END)) / 1000000.0
        ELSE
            ((MAX(input_tokens - cached_input_tokens, 0)
                * CASE WHEN input_tokens > 272000 THEN 0.2 ELSE 0.1 END)
              + (MIN(MAX(cached_input_tokens, 0), MAX(input_tokens, 0))
                * CASE WHEN input_tokens > 272000 THEN 0.02 ELSE 0.01 END)
              + (MAX(output_tokens, 0)
                * CASE WHEN input_tokens > 272000 THEN 0.75 ELSE 0.5 END)) / 1000000.0
    END AS new_cost_usd
FROM matched;

CREATE TEMP TABLE cm_gpt6_sol_luna_account_cost_deltas AS
SELECT
    account_id,
...
FROM (
    ...
    FROM cm_gpt6_sol_luna_request_repriced repriced
    ...
    SELECT
        ...
        CASE WHEN account.secondary_window_request_count = account.request_count
            THEN repriced.new_cost_usd - repriced.old_cost_usd ELSE 0.0 END
    FROM cm_gpt6_sol_luna_rollup_repriced repriced
    JOIN account_usage_billing_stats account ON account.account_id = repriced.account_id
    WHERE repriced.account_id IS NOT NULL
) deltas
GROUP BY account_id;
...
SET estimated_cost_usd = (
    SELECT new_cost_usd
    FROM cm_gpt6_sol_luna_request_repriced
    WHERE cm_gpt6_sol_luna_request_repriced.id = request_token_stats.id
)
WHERE id IN (SELECT id FROM cm_gpt6_sol_luna_request_repriced);
...
SET
    estimated_cost_usd = MAX(estimated_cost_usd + COALESCE((
        SELECT cost_delta FROM cm_gpt6_sol_luna_account_cost_deltas
        WHERE cm_gpt6_sol_luna_account_cost_deltas.account_id = account_usage_billing_stats.account_id
    ), 0.0), 0.0),
    primary_window_estimated_cost_usd = MAX(primary_window_estimated_cost_usd + COALESCE((
        SELECT primary_cost_delta FROM cm_gpt6_sol_luna_account_cost_deltas
        WHERE cm_gpt6_sol_luna_account_cost_deltas.account_id = account_usage_billing_stats.account_id
    ), 0.0), 0.0),
    secondary_window_estimated_cost_usd = MAX(secondary_window_estimated_cost_usd + COALESCE((
        SELECT secondary_cost_delta FROM cm_gpt6_sol_luna_account_cost_deltas
        WHERE cm_gpt6_sol_luna_account_cost_deltas.account_id = account_usage_billing_stats.account_id
    ), 0.0), 0.0)
WHERE account_id IN (SELECT account_id FROM cm_gpt6_sol_luna_account_cost_deltas);

DROP TABLE temp.cm_gpt6_sol_luna_account_cost_deltas;
DROP TABLE temp.cm_gpt6_sol_luna_rollup_repriced;
DROP TABLE temp.cm_gpt6_sol_luna_request_repriced;
